...
saving_var_plots <- function(simple_em_out,    # the 1d Simple Emulator output matrix
  ...
  plot(xP, rep(sigma^2, length(xP)), ylim=ylim_var, type="l", col=col_vec[1], lwd=3,
       ...
       main=bquote(bold("The Variance of the Prior and of the Simple and Derivative Emulators")),
       cex.main=2, cex.lab=1.7, cex.axis=1.4, mgp = c(2.5, 0.8, 0))
  ...
  if(plot_xD){
    ...
    legend("topleft", legend=c(bquote("Prior Variance" ~ sigma^2), 
                                bquote("Variance of the Simple Emulator" ~ Var[D] * "[f(x)]"),
                                bquote("Variance of the Derivative Emulator" ~ Var[D] * "[f(x)]"),
                                "Model Evaluations"),
           lty=c(1,1,1,NA), pch=c(NA, NA, NA, 16), col=c(col_vec, xD_col), 
           lwd=c(2.5, 2.5, 2.5, NA), pt.cex=1.3, cex=1.5)
  }else{
    legend("topleft", legend=c(bquote("Prior Variance" ~ sigma^2), 
                                bquote("Variance of the Simple Emulator" ~ Var[D] * "[f(x)]"),
                                bquote("Variance of the Derivative Emulator" ~ Var[D] * "[f(x)]")),
           lty=c(1,1,1), pch=c(NA, NA, NA), col=col_vec,
           lwd=c(2.5, 2.5, 2.5), cex=1.5)
  }
  ...
  plot(xP, rep(sigma, length(xP)), ylim=ylim_sd, type="l", col=col_vec[1], lwd=3,
       ...
       main=bquote(bold(atop("The Standard Deviation of the Prior and",
       "of the Simple and Derivative Emulators"))),
       cex.main=2, cex.lab=1.7, cex.axis=1.4, mgp = c(2.5, 0.8, 0))
  lines(xP, sqrt(simple_em_out[,"VarD_f(x)"]), col=col_vec[2], lwd=3)
  lines(xP, sqrt(deriv_em_out[,"VarD_f(x)"]), col=col_vec[3], lwd=3)
  if(plot_xD){
    points(xD, rep(0, length(xD)), pch=21, col=1, bg=xD_col, cex=1.5)
    legend("topleft", legend=c(bquote("Prior Standard Deviation" ~ sigma), 
                                bquote("Standard Deviation of the Simple Emulator" ~ SD[D] * "[f(x)]"),
                                bquote("Standard Deviation of the Derivative Emulator" ~ SD[D] * "[f(x)]"),
                                "Model Evaluations"),
           lty=c(1,1,1,NA), pch=c(NA, NA, NA, 16), col=c(col_vec, xD_col), 
           lwd=c(2.5, 2.5, 2.5, NA), pt.cex=1.3, cex=1.5)
  }else{
...
saving_fxD_plots <- function(em_out,                    # the emulator output matrix
  ...
  #   SD_main <- bquote(bold(.(EmulatorType) ~ "Emulator Diagnostics " ~ S[D] * "[f(x)]"))
  # }else if(EmulatorType=="Derivative"){
  #   ED_main <- bquote(bold( atop(.(EmulatorType) ~ "Emulator", 
  #                                "Adjusted Expectation " ~ E[D] * "[f(x)]") ))
  #   VarD_main <- bquote(bold( atop(.(EmulatorType) ~ "Emulator", 
  #                                  "Adjusted Variance " ~ Var[D] * "[f(x)]") ))
  #   SD_main <- bquote(bold( atop(.(EmulatorType) ~ "Emulator", 
  #                                "Diagnostics " ~ S[D] * "[f(x)]") ))
  # }
  ...
  ED_fx_plot <- emul_fill_cont(cont_mat=ED_fx_mat, 
                               ...
                               cex.main=2, cex.lab=1.7, cex.axis=1.4, mgp = c(2.5, 0.8, 0),
                               key.axes = axis(4, cex.axis = 1.4))
  title(main = bquote(bold(.(EmulatorType) ~ "Emulator Adjusted Expectation " ~ E[D] * "[f(x)]")),
        cex.main = 2, outer=TRUE, line=-2.2)
  # title(main = bquote(bold(.(EmulatorType) ~ "Emulator")),
  #       line = 3, cex.main = 2)
  # title(main = expression(bold("Adjusted Expectation " ~ E[D] * "[f(x)]")),
  #       line = 1, cex.main = 2)
  dev.off()
  ...
  VarD_fx_plot <- emul_fill_cont(cont_mat=VarD_fx_mat, 
                                 ...
                                 cex.main=2, cex.lab=1.7, cex.axis=1.4, mgp = c(2.5, 0.8, 0),
                                 key.axes = axis(4, cex.axis = 1.4))
  title(main = bquote(bold(.(EmulatorType) ~ "Emulator Adjusted Variance " ~ Var[D] * "[f(x)]")),
        cex.main = 2, outer=TRUE, line=-2.2)
  # title(main = bquote(bold(.(EmulatorType) ~ "Emulator")),
  #       line = 3, cex.main = 2)
  # title(main = expression(bold("Adjusted Variance " ~ Var[D] * "[f(x)]")),
  #       line = 1, cex.main = 2)
  dev.off()
  ...
  SD_fx_plot <- emul_fill_cont(cont_mat=S_diag_mat, cont_levs=seq(-3,3,0.25), 
                               ...
                               cex.main=2, cex.lab=1.7, cex.axis=1.4, mgp = c(2.5, 0.8, 0),
                               key.axes = axis(4, cex.axis = 1.4))
  title(main = bquote(bold(.(EmulatorType) ~ "Emulator Diagnostics " ~ S[D] * "[f(x)]")),
        cex.main = 2, outer=TRUE, line=-2.2)
  # title(main = bquote(bold(.(EmulatorType) ~ "Emulator")),
  #       line = 3, cex.main = 2)
  # title(main = expression(bold("Diagnostics " ~ S[D] * "[f(x)]")),
  #       line = 1, cex.main = 2)
  dev.off()
  ...
  emul_fill_cont(cont_mat=fxP_mat, 
                 ...
                 cex.main=2, cex.lab=1.7, cex.axis=1.4, mgp = c(2.5, 0.8, 0),
                 key.axes = axis(4, cex.axis = 1.4))
  title(main = bquote(bold("True Computer Model Function f(x)")),
        cex.main = 2, outer=TRUE, line=-2.2)
  dev.off()
}
...
saving_boundary_with_runs_plots <- function(em_out,                    # the emulator output matrix
  ...
  if(EmulatorType =="Boundary"){
    ...
    SD_main <- bquote(bold(.(EmulatorType) ~ "Emulator Diagnostics " ~ S[K] * "[f(x)]"))
  }else if(EmulatorType == "Boundary with Runs"){
    ED_main1 <- bquote(bold(.(EmulatorType) ~ "Emulator")) 
    ED_main2 <- expression(bold("Adjusted Expectation " ~ E[D] * "[f(x)]"))
    VarD_main1 <- bquote(bold(.(EmulatorType) ~ "Emulator"))
    VarD_main2 <- expression(bold("Adjusted Variance " ~ Var[D] * "[f(x)]"))
    SD_main1 <- bquote(bold(.(EmulatorType) ~ "Emulator"))
    SD_main2 <- bquote(bold("Diagnostics" ~ S[D] * "[f(x)]"))
  }else if(EmulatorType == "Boundary with Derivative on the Boundary"){
    ED_main1 <- bquote(bold(.(EmulatorType) ~ "Emulator")) 
    ED_main2 <- expression(bold("Adjusted Expectation " ~ E[K] * "[f(x)]"))
    VarD_main1 <- bquote(bold(.(EmulatorType) ~ "Emulator"))
    VarD_main2 <- expression(bold("Adjusted Variance " ~ Var[K] * "[f(x)]"))
    SD_main1 <- bquote(bold(.(EmulatorType) ~ "Emulator"))
    SD_main2 <- expression(bold("Diagnostics" ~ S[K] * "[f(x)]"))
  }else if(EmulatorType == "Boundary with Derivative on the Boundary and Runs"){
    ED_main1 <- bquote(bold(.(EmulatorType))) 
    ED_main2 <- expression(bold("Emulator Adjusted Expectation " ~ E[D] * "[f(x)]"))
    VarD_main1 <- bquote(bold(.(EmulatorType)))
    VarD_main2 <- expression(bold("Emulator Adjusted Variance " ~ Var[D] * "[f(x)]"))
    SD_main1 <- bquote(bold(.(EmulatorType)))
    SD_main2 <- expression(bold("Emulator Diagnostics" ~ S[D] * "[f(x)]"))
  }
  ...
  if(EmulatorType=="Boundary"){
    ...
    ED_fx_plot <- emul_fill_cont(cont_mat=ED_fx_mat, 
                                 ...
                                 cex.main=2, cex.lab=2, cex.axis=1.4, mgp = c(2.5, 0.5, 0.5),
                                 key.axes = axis(4, cex.axis = 1.4))
  ...
  }else if(EmulatorType %in% c("Boundary with Derivative on the Boundary", 
                               "Boundary with Derivative on the Boundary and Runs")){
  ...
  emul_fill_cont(cont_mat=fxP_mat, 
                 ...
                 cex.main=2, cex.lab=2, cex.axis=1.4, mgp = c(2.5, 0.5, 0.5),
                 key.axes = axis(4, cex.axis = 1.4))
  title(main = "True Computer Model Function f(x)", cex.main = 2, outer=TRUE, line=-2.6)
  dev.off()
}
